"""Database router for the consolidated (monolith) Django project.
...
that there is never a cross-database query or FK.
    ...
    ------------------------------------------
    entities               ->  "nes"        (nes_service.entities)
    courts                 ->  "ngm"        (ngm_service.courts)
    materials              ->  "ngm"        (ngm_service.materials)
    <everything else>      ->  "default"    (jawafdehi: cases / review +
                                             django.contrib.*)

Cross-app access stays in-process but is NEVER a DB join: e.g. the
``cases.services.nes_resolver`` seam queries ``nes_service.entities`` models,
