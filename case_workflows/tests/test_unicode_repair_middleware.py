...
def _request(tool_name: str, args: dict, tool_call_id: str = "tool-1") -> ToolCallRequest:
...
class TestRepairHelpers:
    def test_repair_unicode_escapes_decodes_literal_escape_sequences(self):
        repaired, changed = _repair_unicode_escapes(r"\u0928\u092e\u0938\u094d\u0924\u0947")
...
class TestUnicodeEscapeRepairMiddleware:
    ...
    async def test_awrap_tool_call_passes_repaired_request_to_handler(self):
        ...
        handler = AsyncMock(
            return_value=ToolMessage(content="ok", tool_call_id="tool-1", name="write_file")
        )
