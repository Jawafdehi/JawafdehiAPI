"""OIDC (Zitadel) JWT authentication for the Jawafdehi DRF API.
...
Migration note (DRF token auth removal / chat service account):
    ``config.auth.ChatServiceAccountAuthentication`` (the ``chat-jawafdehi-org``
...
# create_groups.py use (Admin, Moderator, Contributor, ReadOnly, ReviewAssistant)
# plus the NGM rate-limit tier groups.
