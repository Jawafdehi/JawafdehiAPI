...
Observed live contract (2026-06-28, monolith on :48000):
  * ``/api/``            -> 200 browsable root listing the public routers
                            ``{"cases": ..., "sources": ...}`` (NOTE: there is no
                            longer a Jawafdehi ``entities`` router — the unified
                            entity surface is NES at ``/api/nes/entities``).
...
def test_api_root_reachable(client):
    ...
    It advertises the public routers; ``cases`` is the headline surface. The
    monolith root no longer carries an ``entities`` router (entities live under
    ``/api/nes/`` now).
    """
    r = client.get("/api/")
    skip_if_throttled(r)
...
def test_jawafdehi_entities_router_is_gone(client):
    """Jawafdehi no longer serves ``/api/entities/`` — entities are NES-owned.

    The unified entity surface moved to ``/api/nes/entities``; the old Jawafdehi
    ``/api/entities/`` router was removed, so it 404s now.
    """
    r = client.get("/api/entities/")
    skip_if_throttled(r)
    assert r.status_code == 404, (
        f"expected /api/entities/ to be gone (404), got {r.status_code}: {r.text[:200]}"
    )
