...
class CaseAdmin(UserFullNameAdminMixin, admin.ModelAdmin):
    ...
    def title_with_view_link(self, obj):
        ...
        if obj.state in viewable_states and obj.slug:
            ...
            return format_html(
                ...
                'style="margin-left: 8px; font-size: 0.85em;">View on Site</a>',
                obj.title,
                public_url,
            )
        return obj.title
    ...
    title_with_view_link.admin_order_field = "title"

    def contributors_list(self, obj):
        """Display the list of contributors assigned to this case."""
        contributors = obj.contributors.all()
        return ", ".join(
            c.get_full_name() if c.get_full_name() else c.username
