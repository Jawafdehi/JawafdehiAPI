...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for case in cases:
            try:
                ...
                self.stats["errors"] += 1
                logger.exception(f"Error processing case {case.slug}: {e}")
                self.stdout.write(
                    self.style.ERROR(f"✗ Error processing {case.slug}: {e}")
                )
    ...
    def process_case(self, case: Case, dry_run: bool, source_lookup: dict):
        ...
        self.stdout.write(
            f"\n[{self.stats['cases_processed']}] Processing: {case.slug} - {case.title[:80]}..."
        )
