...
class Command(BaseCommand):
    ...
    def _get_ciaa_cases(
        ...
        all_cases = []
        candidate_count = 0
        for case in queryset.order_by("slug"):
    ...
    def _process_case(
        ...
        self.stdout.write(f"\n[{idx}/{total}] {case.slug} — {case.title[:80]}")
    ...
    def _save_timeline(self, case: Case, entries: list[dict], force: bool = False):
        ...
        with transaction.atomic():
            ...
            if not updated:
                raise CommandError(
                    f"Case {case.slug} was populated concurrently; skipping save."
                )
        logger.info("  Saved %d timeline entries to %s", len(entries), case.slug)
