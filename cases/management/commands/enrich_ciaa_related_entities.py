...
Extracts entities via LLM from press releases and court orders and binds those
that link to a NES entity (by canonical nes_id) to the case; entities without a
NES link are skipped (NES owns entities; there is no display-name fallback).
Historically this created local JawafEntity rows; it now creates only
...
class Command(BaseCommand):
    ...
    def _get_accused_relationships(self, case):
        ...
        Returns a list of (relationship, display_name) tuples. The bind holds
        only the NES entity id, so display names are resolved from NES
        in-process (see cases.services.nes_resolver). Binds whose name cannot be
        resolved are still returned with their nes_id as the fallback name so
        overlap detection has something to compare.
        """
        ...
        resolved = resolve_entities(rel.nes_id for rel in rels)
        result = []
        for rel in rels:
            name = (resolved.get(rel.nes_id, {}).get("display_name") or "").strip()
            result.append((rel, name or rel.nes_id))
        return result
    ...
    def _apply_entities(
        ...
        for item in entities_data:
            ...
            # NES is the single source of truth for entities and a bind requires
            # a valid canonical NES id — there is no display-name fallback. If
            # the extracted name can't be linked to a NES entity, we cannot
            # create a bind; skip it (the private/unlinked party would have to be
            # registered in NES first).
            if not is_valid_entity_iri(nes_id or ""):
                self.stats["entities_skipped_no_nes_id"] = (
                    self.stats.get("entities_skipped_no_nes_id", 0) + 1
                )
                continue
