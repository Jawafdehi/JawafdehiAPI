...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--case-id",
            type=str,
            help="Process a specific case by slug",
        )
        parser.add_argument(
            "--priority",
            action="store_true",
    ...
    def handle(self, *args, **options):  # noqa
        ...
        if case_id:
            cases = Case.objects.filter(slug=case_id, case_type=CaseType.CORRUPTION)
        else:
    ...
    def _build_llm_client(self, base_url: str, api_key: str, model: str):
        """Build a chat client on the raw ``openai`` SDK (langchain-openai dropped).

        Returns a thin adapter that preserves the ``.invoke(prompt) -> obj.content``
        interface ``TagEnricher`` expects (the same shape langchain's ChatOpenAI
        exposed), so the enricher call site is unchanged. ``base_url`` makes this
        ...
        class _ChatAdapter:
            """Minimal ChatOpenAI-compatible shim: ``.invoke(str) -> .content``."""
