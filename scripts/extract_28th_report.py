        ...
        if m:
            ...
            if len(parts) >= 4:
                ...
                summary[case_type] = {
                    ...
                    "defendants_male": arabic_int(parts[3]) if len(parts) > 3 else None,
                    "defendants_female": (
                        arabic_int(parts[4]) if len(parts) > 4 else None
                    ),
                    "defendants_total": (
                        arabic_int(parts[5]) if len(parts) > 5 else None
                    ),
                    "bigo_amount_npr": (
                        parse_bigo(parts[-1]) if len(parts) > 1 else None
                    ),
                }
...
if __name__ == "__main__":
    ...
    output_path = (
        sys.argv[2]
        if len(sys.argv) > 2
        else "/paperspace/tmp/corruption-case-db/extracted-data/ciaa-28th-2074-75.json"
    )
