"""One-shot sweep: dead-letter duplicate QUEUED case_review jobs per case slug.

Before ``_enqueue_review_job`` learned to supersede-on-enqueue (and
``regrade_all`` learned to regrade only the latest review per slug), repeated
...
This command collapses that backlog: for every slug it keeps ONLY the newest
...
class Command(BaseCommand):
    help = "Keep only the newest queued case_review job per slug; dead-letter the rest."
    ...
    def handle(self, *args, **opts):
        ...
        for job_id, payload in (
            ...
            if slug:
                by_slug[slug].append(job_id)

        dupes = {slug: ids for slug, ids in by_slug.items() if len(ids) > 1}
        total_stale = sum(len(ids) - 1 for ids in dupes.values())
        self.stdout.write(
            f"queued case_review jobs: {sum(len(v) for v in by_slug.values())} "
            f"over {len(by_slug)} slug(s); {total_stale} stale duplicate(s) "
            f"across {len(dupes)} slug(s)"
        )

        superseded = 0
        for slug, ids in sorted(dupes.items()):
            keep = ids[-1]  # newest enqueue wins
            if apply:
                n = supersede_older_queued_jobs(slug, keep_job_id=keep)
                superseded += n
                self.stdout.write(f"  {slug}: kept job {keep}, superseded {n}")
            else:
                self.stdout.write(
                    f"  {slug}: would keep job {keep}, supersede {len(ids) - 1} "
