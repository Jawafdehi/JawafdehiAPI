...
upserts three kinds of records into THIS database:
  ...
  * cases.JawafEntity              (jawaf entities)
  * cases.DocumentSource           (sources)
  * cases.Case (+ CaseEntityRelationship)  (cases and their entity links)
...
class Command(BaseCommand):
    ...
    def _upsert_case(self, case):
        slug = case.get("slug")
        case_id = case.get("case_id") or f"case-{slug}"
        if self.dry:
        ...
        defaults = {
            ...
            "title": case.get("title") or (slug or case_id),
            "short_description": case.get("short_description") or "",
            "thumbnail_url": case.get("thumbnail_url") or "",
            "banner_url": case.get("banner_url") or "",
            "case_start_date": _parse_date(case.get("case_start_date")),
            "case_end_date": _parse_date(case.get("case_end_date")),
        ...
        # Upsert by case_id (stable). Bypass the model's slug-immutability and
        # full_clean by updating fields directly via queryset when it exists.
        existing = Case.objects.filter(case_id=case_id).first()
        if existing is None and slug:
            existing = Case.objects.filter(slug=slug).first()
    ...
    def _upsert_entities_for_case(self, case_obj, case):
        for ent in case.get("entities") or []:
            nes_id = ent.get("nes_id") or None
            display_name = ent.get("display_name") or None
            if not nes_id and not display_name:
                continue
            # Upsert the entity: prefer nes_id as the natural key, else name.
            if nes_id:
                entity, _ = JawafEntity.objects.get_or_create(
                    nes_id=nes_id, defaults={"display_name": display_name}
                )
                if display_name and entity.display_name != display_name:
                    entity.display_name = display_name
                    entity.save()
            else:
                entity = JawafEntity.objects.filter(
                    nes_id__isnull=True, display_name=display_name
                ).first()
                if entity is None:
