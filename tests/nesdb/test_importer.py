...
def entity_payload(
    slug="alice", prefix="person", full_name="Alice Person", sub_type=None
):
    tp = prefix.split("/")[0]
    payload: dict = {
        "slug": slug,
...
class TestFullImport:
    def test_imports_entities_relationships_and_names(self, nes_repo):
        ...
        write_json(
            nes_repo,
            "relationship/person/alice/organization/political_party/party/MEMBER_OF.json",
            relationship,
        )
        ...
        assert NesEntityName.objects.filter(name_en="Alice Person").exists()
        assert "tags" in NesEntity.objects.first().raw_payload
        rel = NesRelationship.objects.get()
        assert rel.relationship_id == rel_id
        assert "source_entity_id" in rel.raw_payload
        assert NesSyncState.objects.get(pk=1).last_commit_hash == head


class TestIncrementalSync:
    def test_upserts_entity_from_git_diff(self, nes_repo):
        write_json(
            nes_repo,
            "entity/person/alice.json",
            entity_payload("alice", full_name="Alice Person"),
        )
        ...
        write_json(
            nes_repo,
            "entity/person/alice.json",
            entity_payload("alice", full_name="Alice Updated"),
        )
        ...
        assert NesEntityName.objects.get().name_en == "Alice Updated"
        assert NesSyncState.objects.get(pk=1).last_commit_hash == head

    def test_deletes_removed_entity(self, nes_repo):
    ...
    def test_handles_rename_as_delete_and_add(self, nes_repo):
        """When --no-renames is used, git outputs renames as D + A lines."""
        write_json(
            nes_repo,
            "entity/person/alice.json",
            entity_payload("alice", full_name="Alice Person"),
        ...
        assert result.commit_hash == head
        assert NesEntity.objects.count() == 1
        assert NesEntity.objects.first().slug == "bob"


class TestWatermark:
    def test_rolls_back_with_import_transaction(self, nes_repo, monkeypatch):
        write_json(nes_repo, "entity/person/alice.json", entity_payload("alice"))
        commit_all(nes_repo, "initial")
        import_full(nes_repo)
        ...
        assert NesEntityName.objects.get().name_en == "Alice Person"


class TestRelationshipDelete:
    def test_uses_old_json_to_preserve_fk_integrity(self, nes_repo):
        alice_id = "entity:person/alice"
        party_id = "entity:organization/political_party/party"
        write_json(nes_repo, "entity/person/alice.json", entity_payload("alice"))
        write_json(
            nes_repo,
            "entity/organization/political_party/party.json",
            entity_payload("party", "organization/political_party", "Party"),
        )
        write_json(
            nes_repo,
            "relationship/person/alice/organization/political_party/party/MEMBER_OF.json",
            relationship_payload(alice_id, party_id, "MEMBER_OF"),
        )
        commit_all(nes_repo, "initial")
        import_full(nes_repo)
...
class TestEntityPrefix:
    def test_match_uses_segment_boundary_not_plain_startswith(self, nes_repo):
        write_json(
            nes_repo,
            "entity/organization/government/federal/ministry.json",
            entity_payload(
                "ministry",
        ...
        write_json(
            nes_repo,
            "entity/organization/government/legacy.json",
            entity_payload(
                "legacy",
        ...
        write_json(
            nes_repo,
            "entity/organization/political_party/nepali-congress.json",
            entity_payload("nepali-congress", "organization/political_party", "NC"),
        )
        commit_all(nes_repo, "initial")
        import_full(nes_repo)

        gov_ids = set(
            entities_with_prefix("organization/government").values_list(
                "entity_id", flat=True
            )
        )
        assert gov_ids == {
            "entity:organization/government/legacy",
            "entity:organization/government/federal/ministry",
        }

        party_ids = set(
            entities_with_prefix("organization/political_party").values_list(
                "entity_id", flat=True
            )
        )
