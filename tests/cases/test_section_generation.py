...
# --- Court-Stage Conditional Section Tests ---
...
class TestSectionReadinessCheck:
    ...
    def test_ja_requires_two_stages(self):
        # special court implies CHARGE_SHEET, so 2 stages = active
        check = SectionReadinessCheck(
            court_cases=["special:076-CR-0456"], evidence_text=""
        )
        assert check.check_section("ja").active
        # With only 1 stage it should be inactive
        check2 = SectionReadinessCheck(
            court_cases=[], evidence_text="अभियुक्तले बयान दिए।"
        )
        assert not check2.check_section("ja").active
...
def test_all_section_keys_includes_all_9():
    assert len(ALL_SECTION_KEYS) == 9
    expected = set(CORE_SECTION_KEYS) | set(COURT_STAGE_KEYS)
    assert set(ALL_SECTION_KEYS) == expected
