...
def create_entities_from_ids(entity_ids):
    ...
    for nid in entity_ids:
        assert is_valid_entity_iri(nid), f"Invalid NES entity id in test: {nid!r}"
...
def create_case_with_entities(**kwargs):
    ...
    Builds CaseEntityRelationship binds from lists of canonical NES entity ids.
    ...
    # Add entity binds using CaseEntityRelationship (holds the nes_id directly)
    ...
    for nes_id in related_entity_ids:
        CaseEntityRelationship.objects.get_or_create(
            case=case, nes_id=nes_id, relationship_type=RelationshipType.RELATED
        )
