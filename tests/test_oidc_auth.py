"""Tests for config.oidc_auth.OIDCAuthentication (Zitadel OIDC resource-server).
...
def test_valid_token_authenticates_and_syncs_groups(groups):
    token = _make_token(
        sub="zitadel-sub-abc",
        roles={"contributor": {"222": "jawafdehi.org"}},
    )
...
def test_valid_token_creates_user_once_and_resyncs_groups(groups):
    ...
    t1 = _make_token(sub="sub-x", roles={"contributor": {"1": "d"}})
    user1, _ = _authenticate(t1)
    assert User.objects.filter(username="sub-x").count() == 1
    assert set(user1.groups.values_list("name", flat=True)) == {"Contributor"}
...
def test_bad_signature_rejected(groups):
    ...
    token = _make_token(key=_WRONG_KEY, roles={"contributor": {"1": "d"}})
...
def test_missing_required_claim_rejected(groups):
    ...
    token = _make_token(roles={"contributor": {"1": "d"}})
...
def test_unknown_roles_ignored(groups):
    token = _make_token(
        sub="unknown-roles",
        roles={"contributor": {"1": "d"}, "not_a_real_role": {"1": "d"}},
    )
    user, _ = _authenticate(token)
    assert set(user.groups.values_list("name", flat=True)) == {"Contributor"}
...
def test_array_wrapped_roles_claim_normalized(groups):
    ...
    token = _make_token(
        sub="array-roles",
        roles=[{"moderator": {"1": "d"}}, {"contributor": {"1": "d"}}],
    )
    user, _ = _authenticate(token)
    assert set(user.groups.values_list("name", flat=True)) == {
        "Moderator",
...
def test_synced_groups_satisfy_existing_predicates(groups):
    ...
    from cases.rules.predicates import has_role, is_admin, is_contributor

    token = _make_token(sub="pred-user", roles={"contributor": {"1": "d"}})
    user, _ = _authenticate(token)

    # Re-fetch to clear any cached group relation.
    user = User.objects.get(pk=user.pk)
    assert is_contributor(user) is True
