...
class TestNewsletterSubscription:
    ...
    def test_last_name_is_optional(self, api_client):
        response = api_client.post(
            "/api/newsletter/subscriptions/",
            {"firstName": "Sita", "email": "sita@example.com"},
            format="json",
        )
    ...
    def test_resubscribe_updates_existing_record(self, api_client):
        ...
        response = api_client.post(
            "/api/newsletter/subscriptions/",
            {"firstName": "Ram", "email": "ram@example.com"},
            format="json",
        )
    ...
    def test_rate_limit_blocks_eleventh_submission(self, api_client):
        for i in range(10):
            response = api_client.post(
                "/api/newsletter/subscriptions/",
                {"firstName": f"User {i}", "email": f"user{i}@example.com"},
                format="json",
        ...
        response = api_client.post(
            "/api/newsletter/subscriptions/",
            {"firstName": "User 11", "email": "user11@example.com"},
            format="json",
            REMOTE_ADDR="192.168.1.20",
        )
