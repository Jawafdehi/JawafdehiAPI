...
def test_flat_list_projects_reviewers_from_usage():
    _review("case-a", case_title="Case A", status="done", result=RESULT_WITH_USAGE)
...
def test_detail_projects_reviewers_from_usage():
    review = _review(
        "case-d", case_title="Case D", status="done", result=RESULT_WITH_USAGE
    )
...
def test_grouped_latest_carries_reviewers():
    _review("case-g", case_title="Case G", status="done", result=RESULT_WITH_USAGE)
...
def test_reviewers_null_when_no_usage_yet():
    ...
    _review("pending-1", case_title="P1", status="pending")
    _review("no-usage", case_title="NU", status="done", result={"overall_score": 10})
...
def test_reviewers_null_on_malformed_result_shapes():
    ...
    _review("m-list", case_title="M", status="done", result=[1, 2])
    _review("m-scalar-usage", case_title="M", status="done", result={"token_usage": 7})
    _review(
        "m-scalar-bp",
        case_title="M",
        status="done",
