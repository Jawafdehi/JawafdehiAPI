...
`convert_date` tool. Three reasons, in order of weight:
  ...
  3. `casework/common/llm_cache.py` deliberately does not wrap
     `invoke_with_tools` (a multi-turn tool loop has no stable cache key), so
     the donor's shape would make the single most expensive call in the whole
     casework pipeline the one call that can never be served from cache. A dry
     run bills exactly like an apply, so that is the difference between
     iterating on this prompt and paying full premium price for every re-run.
...
def main(argv=None):
    ...
    # Local dev response cache (see casework/common/llm_cache.py). This stage is
    # the most expensive in the pipeline and the one most often re-run after a
    # parse or prompt fix, which is precisely what the cache is for -- and why
    # deviation 2 keeps the main call on `invoke_text`, the wrappable one.
    ...
    # Unconditional, outside the `if usage.calls` guard: a run served entirely
    # from cache makes zero calls, and that is exactly the run whose provenance
    # must not be silent.
