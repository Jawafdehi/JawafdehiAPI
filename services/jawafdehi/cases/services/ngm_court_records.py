...
were retired in the monolith collapse. NGM's courts/cases now live as the
``ngm_service.courts`` app in this same project, routed to the ``ngm`` database
by ``monolith.config.db_router``. So a Jawafdehi management command that needs
...
def get_court_case_details(
    ...
    proxy helper, but reads the ``ngm_service.courts`` models in-process.
    """
    ...
    from ngm_service.courts.models import CaseEntity, CourtCase, CourtCaseHearing
    from ngm_service.courts.normalize import best_effort_normalize
