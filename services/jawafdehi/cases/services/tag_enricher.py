...
class TagEnricher:
    ...
    def enrich_case(
        ...
        if case_num and total_cases:
            ...
            logger.info(
                f"[{case_num}/{total_cases}] Processing {case.slug}{case_num_str} — {case.title[:70] if case.title else 'No title'}"
            )
        else:
            logger.info(f"Processing {case.slug}...")
            ...
            try:
                ...
                if llm_tags:
                    ...
                    if validated:
                        ...
                        logger.info(
                            f"+ Enriched {case.slug} (source_llm): {all_tags}"
                        )
        ...
        all_tags = _append_amount_tag(all_tags, case.bigo)
        logger.info(f"+ Enriched {case.slug} ({tier}): {all_tags}")
        ...
        for idx, case in enumerate(cases_list, start=1):
            ...
            try:
                ...
                if result["status"] == "skipped":
                    stats["skipped"] += 1
                    logger.debug(f"Skipped {case.slug}: {result['reason']}")
                else:
                    stats["enriched"] += 1
                    ...
                    if not dry_run:
                        ...
                        logger.info(
                            f"✓ Saved {case.slug} with {len(result['tags'])} tags"
                        )
                    else:
                        logger.info(f"  [DRY RUN] Would save: {result['tags']}")
            except Exception as e:
                stats["failed"] += 1
                logger.exception(f"Failed to enrich {case.slug}: {e}")
        return stats
