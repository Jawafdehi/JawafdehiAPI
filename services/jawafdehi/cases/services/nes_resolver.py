...
a direct ``iri``-lookup against the NES table. The monolith collapse that would
...
* If the NES app models are importable in this process
  (``nes_service.entities.models.StoredEntity`` — same physical ``entities``
...
def resolve_entities(nes_ids) -> dict[str, ResolvedEntity]:
    ...
    try:
        ...
        # the monolith collapse adds `nes_service.entities` to INSTALLED_APPS and
        ...
        if not _django_apps.is_installed("nes_service.entities"):
            raise ModuleNotFoundError("nes_service.entities not in INSTALLED_APPS")
