...
Talks to the in-process NGM read plane (``ngm_service.courts``), mounted at
``<host>/api/ngm/`` on the same monolith that serves the Jawafdehi API. The old
``/api/ngm/court_case/<court>:<case_number>`` proxy (which forwarded to a
standalone NGM service) was retired in the monolith collapse; this client now
...
def _ngm_base():
    ...
    monolith): the NGM plane is a sibling mounted at ``/api/ngm``. Strip a
