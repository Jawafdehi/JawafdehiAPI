...
def test_get_eligible_cases_skips_non_ciaa():
    ...
    _make_case(slug="non-ciaa", court_cases=["supreme:078-WC-0123"])
    cmd = Command()
    cases = cmd._get_eligible_cases(limit=None, force=False, case_id=None)
    assert len(cases) == 0
...
def test_get_eligible_cases_respects_case_id():
    ...
    _make_case(slug="target")
    _make_case(slug="other")
    cmd = Command()
    cases = cmd._get_eligible_cases(limit=None, force=False, case_id="target")
    assert len(cases) == 1
    assert cases[0].slug == "target"
...
def test_dry_run_does_not_save(
    ...
    case = Case.objects.get(slug="dry-run-test")
...
def test_priority_flag_filters_to_loader_cases(
    ...
    case_yes = Case.objects.get(slug="prio-yes")
    case_no = Case.objects.get(slug="prio-no")
...
def test_all_flag_processes_all_not_just_priority(
    ...
    _make_case(slug="all-a", court_cases=["special:080-CR-0007"], evidence=ev)
    _make_case(slug="all-b", court_cases=["special:999-WC-0001"], evidence=ev)
    ...
    case_a = Case.objects.get(slug="all-a")
    case_b = Case.objects.get(slug="all-b")
