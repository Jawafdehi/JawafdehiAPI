...
class _PublicReadNgmWriteMixin:
    ...
    Mirrors ``nes_service.entities.views.EntityListCreateView.get_permissions``:
...
class QueryView(APIView):
    ...
    def _execute_select(query: str, *, timeout_seconds: float) -> dict:
        ...
        courts models to), NOT the ``default`` connection — after the monolith
...
# app at ``GET /api/search/`` (``monolith.search``), which indexes NGM materials
