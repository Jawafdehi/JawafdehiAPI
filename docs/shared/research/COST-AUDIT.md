...
| **Object storage (local dev)** | MinIO (`quay.io/minio/minio`) | AGPL-3.0 | $0 | **FREE-OSS (WATCH — materialized)** | Free under AGPL, but repo archived/maintenance-mode and admin UI + LDAP/OIDC stripped to paid AIStor in 2025. **2026-09-11: Docker Hub stopped serving `minio/minio` anonymously, which broke the e2e job on every PR; pulls now come from quay.io pinned to the last community release.** Still fine as an *unmodified local-dev S3 stand-in*, but the distribution channel is no longer dependable — consider Garage/SeaweedFS if you want a maintained dev server. See WATCH list. |
