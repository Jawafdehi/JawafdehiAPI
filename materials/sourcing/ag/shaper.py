...
``(jsonld_doc, material_type)`` — the same shape as the NKP shaper. Lives beside
its sourcing pipeline under ``materials/sourcing/ag/`` (see
``materials/sourcing/README.md``); persisted through the material API plane (the
``/file`` upload endpoint for the PDF/markdown, then ``PUT`` the doc), which lands
on :func:`materials.single_source_ingest.upsert_single_source_material`.
"""
...
def ag_abhiyog_to_jsonld(
    ...
    *,
    ...
    ``record`` carries the scrape metadata (from the corpus index / manifest):
      ...
      * ``date_ad``       — pre-converted AD ISO date (optional; overrides the
        BS→AD conversion when the client already holds one).
    ...
    # always preserve it on jawafdehi:filingDateBS. datePublished (schema.org) is
    # the Gregorian date search sorts on: prefer a client-supplied AD date, else
    # convert the BS date via the shared helper, else fall back to the BS string
    # so the date is never dropped even when conversion is unavailable.
    date_bs = str(record.get("created_date_np") or "").strip()
    date_ad = record.get("date_ad") or bs_to_ad_iso(date_bs)
    if date_bs:
