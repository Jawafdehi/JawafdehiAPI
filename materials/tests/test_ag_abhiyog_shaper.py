...
from materials.sourcing.ag_abhiyog import (
    AG_SOURCE,
    SOURCE_TYPE,
    _slug_ident,
...
def test_bs_date_always_preserved_even_without_ad():
    # created_date_np present but no date_ad (converter unavailable) -> BS is kept
    # on filingDateBS AND used as datePublished fallback (date never dropped).
    doc, _ = ag_abhiyog_to_jsonld({"record_id": 86132, "name": "x", "created_date_np": "2082-3-29"})
    assert doc["jawafdehi:filingDateBS"] == "2082-3-29"
    assert doc["datePublished"] == "2082-3-29"
